/**
 * Per-project financial rollup card.
 *
 * Server Component. Admin-only — the project detail page gates this
 * card on the same admin check the rest of the transactions module
 * uses. Staff and company-role viewers don't see the card at all
 * (the page render skips the component entirely).
 *
 * Two halves:
 *   - Per-type breakdown grid (5 type cells with count + paise sum)
 *   - "Recent transactions" mini-list (latest 5 rows, linked to
 *     the transaction detail page)
 *   - "View all" link to /dashboard/transactions?projectId=<id>
 *
 * @module app/dashboard/projects/[id]/_components/project-rollup-card
 */
...
export interface ProjectRollupCardProps {
  projectId: string;
}

export async function ProjectRollupCard({ projectId }: ProjectRollupCardProps) {
  ...
  const [rollupResult, recentResult] = await Promise.all([
    ...
    getProjectRecentTransactions(projectId, 5),
  ]);
    ...
    <Card className="mt-4 overflow-hidden p-0">
      <header className="flex items-center justify-between border-b border-border bg-card p-4">
        ...
        <Button asChild variant="ghost" size="sm">
          <Link
            href={`/dashboard/transactions?projectId=${projectId}`}
        ...
        <section>
          ...
          {recent.length === 0 ? (
            <p className="text-sm italic text-muted-foreground">
              No transactions recorded on this project yet.
            </p>
          ) : (
