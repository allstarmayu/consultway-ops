 ...
 * scoped) and returns a `Uint8Array` of PDF bytes via
 * `@react-pdf/renderer`'s `renderToBuffer`. No DB calls, no session
 * reads — the caller resolves auth + the aggregate payload, then hands
 * the resolved input here.
 *
 * Where this runs (Layer A architecture, Day 31):
 *   `@react-pdf/renderer` is ~5-7 MiB minified — bundling it into the
 *   main OpenNext worker pushed us over Cloudflare's 10 MiB Workers Paid
 *   ceiling. So this module is owned by a DEDICATED sibling worker,
 *   `consultway-ops-pdf` (`workers/pdf/`), which imports it directly and
 *   exposes a `POST /render` handler. The main worker never bundles this
 *   file: it reaches the renderer over a Cloudflare service binding
 *   (`env.PDF_WORKER`) via the dispatcher in `lib/reports/pdf.tsx`.
 *
 *   In local dev / tests (Node) the dispatcher dynamic-imports this
 *   module directly (no service binding available), so PDFs still render
 *   inline. The PDF render unit tests import this module straight.
 *
 * Why the renderer stays a standalone pure builder:
 *   - Unit-testable in isolation (smoke-tested against a synthetic
 *     payload — no DB fixture needed for the rendering contract).
 ...
 *   - Single source of truth for the renderer: both the PDF worker and
 *     the local dispatcher fallback import THIS file — no duplication.
 *
 * Sections rendered:
 ...
 *
 * @module lib/reports/pdf-renderer
 */
import {
  Document,
  Page,
...
// Relative imports (not the project's usual `@/` alias) on purpose: this
// file is bundled by TWO toolchains — Next/OpenNext (for the local dev
// fallback in lib/reports/pdf.tsx) AND wrangler's standalone esbuild pass
// (for the consultway-ops-pdf worker, built from workers/pdf/). The `@/`
// alias resolves reliably in Next but is fragile under wrangler's esbuild
// across the tsconfig `extends` boundary. Relative paths resolve
// identically in both. See docs/DEPLOY_PDF_WORKER.md.
import { formatRupeesFromPaiseAscii } from "../format/inr";
import type {
  ProjectStatus,
  TenderStatus,
  TransactionType,
